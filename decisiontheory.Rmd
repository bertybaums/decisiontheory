...
url: https://bertybaums.github.io/decisionstheory/
documentclass: book
# bibliography:
# - book.bib
# - packages.bib
...
[Download PDF of Entire Textbook](decisiontheory.pdf)
...
<!--chapter:end:03-transitivity-and-completeness.Rmd-->
...
<!--chapter:end:05-expectation.Rmd-->
...
```{tikz, echo=FALSE, fig.cap = "General Causal Pathway for Decisions. The WORLD bubble is like all the columns in our decision tables, the ACTION bubble is like all the rows, and the OUTCOME bubble is like all the cells of a table.", fig.ext=if (knitr:::is_latex_output()) 'pdf' else 'png'}
\usetikzlibrary{shapes,decorations,arrows,calc,arrows.meta,fit,positioning}
\tikzset{
    -Latex,auto,node distance =1 cm and 1 cm,semithick,
    state/.style ={ellipse, draw, minimum width = 0.7 cm},
...
```{tikz, echo=FALSE, fig.cap = "Getting a headache can depend on having spots in your vision or whether you ate a banana.", fig.ext=if (knitr:::is_latex_output()) 'pdf' else 'png'}
\usetikzlibrary{shapes,decorations,arrows,calc,arrows.meta,fit,positioning}
\tikzset{
    -Latex,auto,node distance =1 cm and 1 cm,semithick,
    state/.style ={ellipse, draw, minimum width = 0.7 cm},
...
```{tikz, echo=FALSE, fig.cap = "A common cause pathway. A deficiency of potassium causes both spots in vision and a desire to eat bananas.", fig.ext=if (knitr:::is_latex_output()) 'pdf' else 'png'}
\usetikzlibrary{shapes,decorations,arrows,calc,arrows.meta,fit,positioning}
\tikzset{
    -Latex,auto,node distance =1 cm and 1 cm,semithick,
    state/.style ={ellipse, draw, minimum width = 0.7 cm},
...
This more complicated causal graph can no longer be represented by a single equation. Rather, we now need a *system* of equations that when taken together represent the graph.^[In the literature each line is called a *structural* equation, and the collection of them is the *system* of structural equations.] 
...
If we now consider the possibilities, we'll notice that you never get a headache. This is because on the days that you would get a headache from spotty vision are days when you have a potassium deficiency, but a potassium deficiency also means that you will eat a banana, and thereby counteract the headache. In our example, spotty vision and eating bananas have a **common cause**, namely a potassium deficiency.^[Try out this reasoning on the system of equations: set $D=0$ and find the value of $H$, then do it again when you set $D=1$. What do you notice about the value of $H$ in each case?] 
...
```{tikz, echo=FALSE, fig.cap = "Test Example - Incorrect decision model setup because world states and acts are supposed to be independent, but they are not (CHOICE is a common cause).", fig.ext=if (knitr:::is_latex_output()) 'pdf' else 'png'}
\usetikzlibrary{shapes,decorations,arrows,calc,arrows.meta,fit,positioning}
\tikzset{
    -Latex,auto,node distance =1 cm and 1 cm,semithick,
    state/.style ={ellipse, draw, minimum width = 0.7 cm},
...
```{tikz, echo=FALSE, fig.cap = "More realistic causal graphs of decisions. The idea to represent things in this way come from Kenny Easwaran's 'A classification of Newcomb problems and decision theories' in *Synthese* (2021, 198).", fig.ext=if (knitr:::is_latex_output()) 'pdf' else 'png'}
\usetikzlibrary{shapes,decorations,arrows,calc,arrows.meta,fit,positioning}
\tikzset{
    -Latex,auto,node distance =1 cm and 1 cm,semithick,
    state/.style ={ellipse, draw, minimum width = 0.7 cm},
...
Let's say Bob is more than 50% confident that 
...
<!--chapter:end:08-odds-probabilities-and-action.Rmd-->
...
<!--chapter:end:09-basic-logic-and-probabilities.Rmd-->

# Conditional Probabilities and Likelihoods



<!--chapter:end:10-conditional-probabilities-and-likelihoods.Rmd-->

# Base Rates, Priors, and Bayes Rule

<!--chapter:end:11-base-rates-priors-and-bayes.Rmd-->

# Learning and Motivated Reasoning

<!--chapter:end:12-learning-and-motivated-reasoning.Rmd-->
