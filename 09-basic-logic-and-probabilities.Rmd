...
`r newthought("Independence")` is an important concept of inductive logic. It says that the truth of one proposition *does not change the probability* of another proposition being true. It's important not to confuse independence with exclusivity.^[Note that if A and B are exclusive, then finding out that A is true *does* change the probability that B is true, in fact we know it would ahve to be false! Hence, A and B are not independent.]
